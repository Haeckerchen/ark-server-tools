= ARK: Survival Evolved Linux Server Tools (German Version)

Dieses Tool erlaubt es Dir jegliche ARK: Survival Evolved Linux Server zu verwalten ohne lange Befehle auswendig zu lernen.
Es bietet eine Vielzahl an Funktionen, für die volle Liste schaue in der <<Benutzung>> Sektion nach.

Installation
------------

Vorraussetzungen für ARK Server Tools
~~~~~~~~~~~~~

ARK Server Tools benötigt bestimmte Packages um richtig funktionieren zu können.

Für CentOS oder RHEL musst Du die folgenden Packages installiert haben:
```
perl-Compress-Zlib
curl
lsof
...
Für Debian oder Ubuntu musst Du die folgenden Packages installiert haben:
```
perl-modules
curl
lsof
...
Normalerweise schon installierte Packages sind:
```
>=bash-4.0
>=coreutils-7.6
findutils
...
Für CentOS 6 oder früher, sowie Debian 7 oder früher musst Du eine neue Version von glibc kompilieren.
Anleitung (Englisch): https://github.com/FezVrasta/ark-server-tools/wiki/Install-of-required-versions-of-glibc-and-gcc-on-RHEL-CentOS

Um ARK Server Tools nutzen zu können musst Du **SteamCMD** nach dieser Anleitung (Englisch) installieren:

https://developer.valvesoftware.com/wiki/SteamCMD#Linux

Wir nehmen an, dass Du den Benutzer `steam` angelegt hast um steamcmd und Deine ARK Server zu speichern.

Vorraussetzungen für ARK Server
~~~~~~~~~~~~

Um Deinem ARK Server die Möglichkeit zu geben nach außen erreichbar zu sein musst Du ein paar Ports in der Firewall freigeben:
...
Beachte: Ändere die Ports je nachdem welche Ports Du in der `arkmanager.cfg` Datei eingestellt hast.

Installieren der ARK Server Tools
~~~~~~~~~~~~~~~~~~~~~~~~

Um ARK Server Tools zu installieren führe diesen Befehl aus:

[source,sh]
curl -sL https://raw.githubusercontent.com/Haeckerchen/ark-server-tools/master/netinstall.sh | sudo bash -s steam

Beachte: Du musst den `bash -s` Parameter ändern falls dein Steam-Benutzer anders ist als `steam`.


Das kopiert das `arkmanager` Script und seinen Daemon in die richtigen Verzeichnisse und erstellt einen leeren Log-Ordner in `/var/log` für ARK Server Tools.

Falls Du lieber eine Benutzer-Installation der ARK Server Tools als unpriviligierten Benutzer haben möchtest, führe diesen Befehl aus:

[source,sh]
curl -sL https://raw.githubusercontent.com/Haeckerchen/ark-server-tools/master/netinstall.sh | bash -s -- --me

Konfiguration
~~~~~~~~~~~~~

In `/etc/arkmanager/arkmanager.cfg` und `/etc/arkmanager/instances/<Instanzname>.cfg` gespeichert findest Du die Variablen die benötigt werden um den Server zu starten, wie Ports, System-Umgebungs-Variablen und so weiter.
...
[source,sh]
ark_SessionName="Mein ARK Server"
ark_MaxPlayers=50
ark_ServerPVE=False
